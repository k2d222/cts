...
import { makeListing } from './crawl.ts';
...
app.get('/out/:suite([a-zA-Z0-9_-]+)/webworker/:filepath(*).as_worker.js', (req, res, next) => {
  ...
  const result = `\
import { g } from '/out/${suite}/${filepath}.spec.ts';
import { wrapTestGroupForWorker } from '/out/common/runtime/helper/wrap_for_worker.ts';

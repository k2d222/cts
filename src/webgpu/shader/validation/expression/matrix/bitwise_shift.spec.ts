...
// A list of bitshift operators
const kBitshiftOperators = {
  left: { op: '<<' },
  right: { op: '>>' },
};
...
g.test('invalid')
  ...
  .params(u =>
    u
      .combine('op', keysOf(kBitshiftOperators))
...
fn main() {
  let foo = ${lhs} ${kBitshiftOperators[t.params.op].op} ${rhs};
}
`;

    const pass = (lhs === '1u' || lhs === '1i' || lhs === '1') && (rhs === '1u' || rhs === '1');
